...
#from .common.MaskDecoder import TwoWayTransformer

class Sam2pt5D(nn.Module):
    ...
    def preprocess(self, x: torch.Tensor, is_input=True) -> torch.Tensor:
        ...
        if is_input:
            ...
            x = F.interpolate(
                x.unsqueeze(0), (self.image_encoder.img_size // 4, self.image_encoder.img_size // 4), mode="nearest-exact"
            ).squeeze(0)
        return x
...
def _build_sam2pt5d(
    ...
    if checkpoint is not None:
        ...
        for k, v in state_dict.items():
            ...
            # due to the change of input size
            if k == "image_encoder.patch_embed.proj.weight":
                conv_weight = v
                conv_tipe = v.dtype
                conv_weight = conv_weight.float()
                repeat = int(math.ceil(encoder_in_chans / 3))
                conv_weight = conv_weight.repeat(1, repeat, 1, 1)
                conv_weight *= 3 / float(encoder_in_chans)
                conv_weight = conv_weight.to(conv_tipe)
                new_dict[k] = conv_weight
            else:
                new_dict[k] = v
        ...
        total_trainable_params = sum(
            p.numel() if p.requires_grad else 0 for p in sam.parameters()
        )
        print(
            f"{sam.__class__.__name__} has {total_trainable_params * 1.e-6:.2f} M trainable params."
        )
...
def _build_sam_tiny(
    ...
    if checkpoint is not None:
        ...
        for k, v in state_dict.items():
            # skip weights in position embedding and learned relative positional embeddings
            # due to the change of input size
            if k == "image_encoder.patch_embed.seq.0.c.weight" :
                conv_weight = v
                conv_tipe = v.dtype
                conv_weight = conv_weight.float()
                repeat = int(math.ceil(encoder_in_chans / 3))
        ...
        #    new_dict.pop("image_encoder.patch_embed.seq.0.c.weight")
        #    new_dict.pop("image_encoder.patch_embed.seq.0.c.bias")
...
def build_sam_vit_l(
    mod="sam",
    mask_decoder_config: str = "original",
    checkpoint=None,
    image_size=1024,
    encoder_in_chans=3,
):
...
def build_sam_vit_b(mod='sam', mask_decoder_config: str = 'original',
    checkpoint=None, image_size=1024, encoder_in_chans=3
    ):
    return _build_sam2pt5d(
        mod,
        encoder_in_chans=encoder_in_chans,
